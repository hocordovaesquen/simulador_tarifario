...
def leer_negociacion(archivo):
    ...
    # Header está en fila 6 (índice 6)
    df = pd.read_excel(archivo, sheet_name='A.3 BBDD Neg', header=6)
    
    # Las columnas ya tienen nombres correctos
    col_pais = 'Pais'
    col_broker = 'Cliente estandar'
    col_monto = 'Monto USD'
    
    # Buscar columnas de ingresos (nombres exactos)
    columnas_ingreso = {
        'acceso_real': 'Cobro Acceso',
        'trans_real': 'Cobro Transacción'
    }
