# app.py
# ==================================================================================
# Simulador Tarifario RV — v6.1 (Excel‑driven, safe merged‑cells writes)
# ==================================================================================
...
st.set_page_config(page_title="Simulador Tarifario RV – Excel‑driven (v6.1)", page_icon="📊", layout="wide")
...
def safe_set(ws, coord: str, value):
    """Escribe en una celda; si es parte de un rango merged, usa el anchor (min_row/min_col)."""
    cell = ws[coord]
    try:
        if isinstance(cell, MergedCell):
            # encontrar el rango y escribir en el anchor
            for rng in ws.merged_cells.ranges:
                if coord in rng:
                    anchor = ws.cell(row=rng.min_row, column=rng.min_col)
                    anchor.value = value
                    return
            # si no encontramos rango, no escribir
            return
        else:
            cell.value = value
    except AttributeError:
        # último recurso: ignorar (no reventar)
        pass
...
def write_params_to_wb(wb, params: Dict):
    ...
    def write_block(rows: List[int], data_key: str, field_names: Tuple[str,str,str,str]):
        ...
        for pais, cols in MAP_RIGHT.items():
            ...
            # "limpiar" sólo las filas que vayamos a pisar; no tocar merged headers
            max_rows = len(rows)
            # Escribir lo editado y setear en 0 las filas libres hasta el final del bloque
            lst = block.get(pais, [])
...
# ---------------- UI ----------------
st.title("📊 Simulador Tarifario RV – Excel‑driven (v6.1)")
...
df_cli2 = approx_engine(df_cli, bbdd) if 'approx_engine' in globals() else df_cli
